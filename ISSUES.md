...
>
> **#1~#7 일괄 처리 완료** (2026-04-28~29, 4 커밋: `cc5c3ef` / `e05a23f` / `f99ff44` / `909656c`).
> #6 만 잔여 작업 있음 — 후속 트랙 X-1 (CLAUDE.md "후속 작업 트랙" 참조) 에서 다룸.

---

## #1 (모바일) 햄버거 버튼 위치 — 비로그인 시 좌측으로 쏠림

- **상태**: ✅ 해결 (커밋 `cc5c3ef`, 2026-04-28) — #3 위젯 도입으로 자동 해결 (헤더 우측이 항상 "로그인" 버튼 또는 위젯으로 채워짐)
...
- **상태**: ✅ 적용 (커밋 `cc5c3ef`, 2026-04-28) | 분류: 기능 변경
...
- **상태**: ✅ 적용 (커밋 `cc5c3ef`, 2026-04-28) | 분류: 기능/UX 재구조화
...
- **상태**: ✅ 적용 (커밋 `cc5c3ef`, 2026-04-28) | 분류: UI/UX 개선
...
- **상태**: ✅ 적용 (커밋 `e05a23f`, 2026-04-29) | 분류: UX 변경
- **추가 보완 (사용자 피드백)**:
...
- **상태**: 🔶 1차 적용 (커밋 `909656c`, 2026-04-29) | 분류: 구조 개선
- **남은 작업**: 멤버 관리 리스트 / 쿠폰 히스토리 등 잔여 페이지에 `patchList` 적용
- **후속 처리 방침** (2026-04-29 사용자 결정): 잔여 작업은 단순 `patchList` 확장이 아니라
  웹 프로젝트 설계 전반 재검토와 함께 진행. **CLAUDE.md "후속 작업 트랙 → X-1"** 에서 다룸.
  Phase B/C 신규 콘텐츠 구현 완료 후 트리거.
